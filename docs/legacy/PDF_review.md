...
- The builder (`AppNavigation` plus `AppSurfaceRegistry`) keeps active PDF open/import flows out of the live shell.
...
- `data/books/PdfLegacyBridge.kt`
- `data/books/PdfToEpubConverter.kt`
- `data/books/PdfConversionWorker.kt`
...
2. Re-check the builder registry seam in `AppSurfaceRegistry.kt`.
3. Re-check parser/runtime seams in `EpubParser.kt` and `data/books/PdfLegacyBridge.kt`.
