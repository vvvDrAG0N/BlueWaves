...
## Theme Spectrum Picker Plan Refresh
- Goal: Refresh the picker implementation plan around the newly approved picker-only redesign: a 2D saturation/value square plus a hue control, with guided mode constrained to a real in-square safe zone and Advanced mode left fully unrestricted.
- Why now: The updated design is captured in `docs/superpowers/specs/2026-04-27-theme-spectrum-picker-design.md`, and it intentionally narrows scope versus the older broader spectrum plan. The previous implementation plan should not be executed as-is because it assumed RGB field-token changes and a softer exact-zone teaching model instead of true guided drag boundaries.
- Suggested owner/model: Codex / GPT-5.
- Starting docs/files: `AGENTS.md`, `docs/settings_persistence.md`, `docs/test_checklist.md`, `docs/superpowers/specs/2026-04-27-theme-spectrum-picker-design.md`, `feature/settings/src/main/java/com/epubreader/feature/settings/SettingsThemeColorPicker.kt`, `feature/settings/src/main/java/com/epubreader/feature/settings/ThemeEditorColorEditing.kt`, `feature/settings/src/main/java/com/epubreader/feature/settings/ThemeEditorModels.kt`, `core/model/src/main/java/com/epubreader/core/model/ThemePaletteGeneration.kt`, `feature/settings/src/main/java/com/epubreader/feature/settings/SettingsThemeEditor.kt`
- Risks: Letting the guided safe zone drift away from the real resolver, turning a dynamic allowed region into a fake static shape, reintroducing save-on-dismiss behavior, or accidentally expanding the scope into editor-token or persistence changes.
- Verification target: After the refreshed implementation plan is written, run the scoped settings JVM and instrumentation slice, confirm `checkKotlinFileLineLimit` still passes, and do one manual emulator smoke on Basic/Extended versus Advanced picker behavior.
