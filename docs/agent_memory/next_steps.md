...
- Goal: Execute the refreshed picker-only redesign: a 2D saturation/value square plus a hue slider, with guided mode constrained to the real in-square safe zone and Advanced mode left fully unrestricted.
- Why now: The narrowed design is captured in `docs/superpowers/specs/2026-04-27-theme-spectrum-picker-design.md`, and the stale broader implementation plan has now been replaced by the refreshed execution plan at `docs/superpowers/plans/2026-04-27-theme-spectrum-picker.md`.
- Suggested owner/model: Codex / GPT-5.
...
- Risks: Letting the guided safe zone drift away from the real resolver, turning a dynamic allowed region into a fake static shape, reintroducing save-on-dismiss behavior, or accidentally expanding the scope beyond the picker surface.
- Verification target: Execute the scoped settings JVM and instrumentation slice from the refreshed plan, confirm `checkKotlinFileLineLimit` still passes, and do one manual emulator smoke on Basic/Advanced picker behavior before closing the work.
