...
object EditBookSurfacePlugin : SurfacePlugin<EditBookRoute, EditBookDependencies, EditBookEvent> {
    override val surfaceId: SurfaceId = SurfaceId("edit_book")

    override fun decodeRouteArgs(routeArgs: Any?): SurfaceRouteDecodeResult<EditBookRoute> {
        val args = routeArgs as? EditBookRouteArgs
            ?: return SurfaceRouteDecodeResult.Failure("Edit Book route requires a book ID.")
        if (args.bookId.isBlank()) {
            return SurfaceRouteDecodeResult.Failure("Edit Book route requires a book ID.")
        }
        return SurfaceRouteDecodeResult.Success(EditBookRoute(bookId = args.bookId))
    }
