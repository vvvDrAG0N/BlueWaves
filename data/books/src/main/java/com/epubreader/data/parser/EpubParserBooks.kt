...
internal const val EPUB_COVER_FILE_NAME = "cover_thumb.png"
...
internal const val PDF_COVER_WIDTH_PX = 320
...
private fun reconcileStoredBookCoverFiles(
    ...
    val coverEntry = coverHref?.let { href ->
        val archivePath = InternalEpubParser.resolveZipPath(opfDir, href).substringBefore("#")
        zip.getEntry(archivePath)
    }
...
internal fun isEditedBookCustomCoverHref(rawHref: String): Boolean {
    return rawHref
        .substringBefore("#")
        .removePrefix("/")
        .trim()
